...
wording differences with no structural implication; anything you are not confident is a real \
gap. When in doubt, do not invent a finding -- an empty findings list is a correct, expected \
result for a clean diagram.

For every gap you find, produce ONE finding: a specific, plain-English question a Process Analyst \
could answer without knowing any internal BPMN ids (e.g. "'Classify requirements' has no step \
before it -- is this where the process starts?" not "el_1aa2... has no incoming flow"), the \
element/flow ids it concerns (target_element_ids), and 1-3 concrete resolution options. Every \
option needs a diff shaped EXACTLY like the schema below and constructed using these rules \
(identical to how diagram edits are made elsewhere in this project):
...
def build_system_prompt(schema: ProcessSchema) -> str:
    ...
    return _INSTRUCTIONS.replace("##SCHEMA_JSON##", f"Current process schema (JSON):\n{schema_json}")
